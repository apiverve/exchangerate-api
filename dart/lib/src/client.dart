...
class ExchangerateValidationException implements Exception {
  final List<String> errors;

  ExchangerateValidationException(this.errors);

  @override
  String toString() => 'ExchangerateValidationException: ${errors.join("; ")}';
}
...
///
/// For more information, visit: https://exchangerate.apiverve.com?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [currency1] (required) - The first currency you want to get the exchange rate for
/// * [currency2] (required) - The second currency you want to get the exchange rate for
class ExchangerateClient {
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'currency1': ValidationRule(type: 'string', required: true),
    'currency2': ValidationRule(type: 'string', required: true),
  };

  ExchangerateClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/exchangerate',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [ExchangerateValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw ExchangerateValidationException(errors);
    }
  }

  /// Execute a request to the Exchange Rate API.
  ///
  /// Parameters are validated before sending the request.
  Future<ExchangerateResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw ExchangerateException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw ExchangerateException('Resource not found');
      } else {
        throw ExchangerateException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is ExchangerateException) rethrow;
      throw ExchangerateException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Exchange Rate API client.
